...
#class for maintaining linked list
    ...
    : self.head = None self.tail = None self.size = 0

#add a node at the last of a linked list
    def addNode(self, data)
    : temp_node = Node(data) if (self.head == None)
    : #if size of list is 0 self.head = temp_node self.tail = temp_node else : #size of list in not zero
                                                                               self.tail.next = temp_node self.tail = temp_node

#print the whole linked list
    def print(self)
    : temp_node = self.head
                  output = "" while (temp_node != None)
    : output = output + str(temp_node.data) + "->" temp_node = temp_node.next
                                                               print(output)
...
#returns previous node of the node 'n'
    ...
    ''' create the partition of the list all the number less than pivot is on left side of pivot
...
, 6
, 5
, 4 ''' def divideList(self, start, end)
    : pivot = end.data
              i = None j = start while j != end : if j.data <= pivot :
#increment index of smaller element
            if i
        == None : i = start else : i = i.next
...
#increment index of smaller element
    ...
    : if start != end and start != None and end != None and start != end.next : pivot = self.divideList(start, end) #create partition of unsorted list
